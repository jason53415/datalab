...
unsigned floatScale64(unsigned uf)
{
    ...
    if (exp == 0) {
        ...
        while (!(uf & (1 << count)))
            count = count - 1;
        uf = uf << (23 - count);
        return sign | (uf & 0x807fffff) | ((count - 16) << 23);
    }
    if (exp == 255)
        return uf;
    exp = exp + 6;
...
{
    int mask1 = 0x1 << 30 << 1 >> n << 1;
    int mask2 = (0x1u << n) + ~0x0;
    return (x << n) | (((x & mask1) >> (33 + ~n)) & mask2);
}
